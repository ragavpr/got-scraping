...
import "got";
import http23 from "http2-wrapper";
import "node:url";
...
// src/hooks/proxy.ts
import "got";
import http22, { auto } from "http2-wrapper";
...
// src/agent/socks/socks-http2-agent.ts
import { Agent as Agent2 } from "http2-wrapper";
import "tls";
import "url";
import { SocksClient } from "socks";
var _SocksHttp2Agent = class _SocksHttp2Agent extends Agent2 {
  constructor(options) {
    super(options);
    __publicField(this, "proxy");
...
__name(_SocksHttp2Agent, "SocksHttp2Agent");
var SocksHttp2Agent = _SocksHttp2Agent;

// src/agent/socks/http-socks-agent.ts
import { SocksProxyAgent } from "socks-proxy-agent";
var _HttpSocksAgent = class _HttpSocksAgent extends SocksProxyAgent {
  get protocol() {
    return "http:";
  }
  set protocol(value) {
...
__name(_HttpSocksAgent, "HttpSocksAgent");
var HttpSocksAgent = _HttpSocksAgent;

// src/agent/socks/https-socks-agent.ts
import { SocksProxyAgent as SocksProxyAgent2 } from "socks-proxy-agent";
var _HttpsSocksAgent = class _HttpsSocksAgent extends SocksProxyAgent2 {
  get protocol() {
    return "https:";
  }
  set protocol(value) {
...
__name(_HttpsSocksAgent, "HttpsSocksAgent");
var HttpsSocksAgent = _HttpsSocksAgent;
...
async function proxyHook(options) {
  ...
  if (proxyUrl) {
    const parsedProxy = new URL3(proxyUrl);
...
// src/resolve-protocol.ts
import { SocksClient as SocksClient2 } from "socks";
var connectSocks = /* @__PURE__ */ __name(async (proxyUrl, options, callback) => {
  const url = new URL4(proxyUrl);
...
var connectHttp = /* @__PURE__ */ __name(async (proxyUrl, options, callback) => new Promise((resolve, reject) => {
...
var getResolveProtocolFunction = /* @__PURE__ */ __name((options, proxyUrl, sessionData) => {
  ...
  return (...args) => http23.auto.resolveProtocol({
    ...args[0],
...
var refererHook = /* @__PURE__ */ __name((options, response) => {
  const url = options.url;
  const resUrl = new URL7(response.url);
